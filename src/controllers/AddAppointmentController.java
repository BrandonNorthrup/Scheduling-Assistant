...
public class AddAppointmentController implements Initializable {
    ...
    public boolean handleAddAppointment(int id) {
        ...
        if(DatabaseAppointment.overlappingAppointment(-1, location.getText(), ld.toString(), times.get(aptTime))) {
            errors.add("Overlapping Appointments");
            return false;
        }
        if(DatabaseAppointment.saveAppointment(id, types.get(aptType), contacts.get(aptContact), location.getText(), ld.toString(), times.get(aptTime))) {
            return true;
        } else {
